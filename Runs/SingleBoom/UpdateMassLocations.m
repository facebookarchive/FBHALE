...
%% Batteries
optim.batteries.xCG_m(2) = optim.wing.x_m + optim.wing.structure.sRefX_m{1}(fix(optim.batteries.yobo2(2)*optim.wing.structure.numStations)) ...
                                    + optim.wing.structure.Cea_m{1}(fix(optim.batteries.yobo2(2)*optim.wing.structure.numStations));
...
saxis   = interp1(optim.wing.structure.sRefY_m{1}, optim.wing.structure.sRefX_m{1}+optim.wing.x_m , optim.propulsion.y_m);
chord   = interp1(optim.wing.structure.sRefY_m{1}, optim.wing.structure.chord_m{1} , optim.propulsion.y_m);
xax     = interp1(optim.wing.structure.sRefY_m{1}, optim.wing.structure.Xax{1}, optim.propulsion.y_m);
Le      = saxis-chord.*xax;
...
optim.vtail.y_m = 0;
% Set vtail height
optim.vtail.z_m = (optim.vtail.bref_m/2)-3*max(optim.boom.omlSectionsR_m{1}); % set tail so that with landing gear 3 times fuseage radius tail touches at 0 alpha

%% Pod locations
optim.pod.x_m = optim.batteries.xCG_m(2)-optim.pod.structure.length_m/2;
optim.pod.y_m = optim.batteries.y_m(2);
optim.pod.zCG_m = optim.batteries.zCG_m(2);
optim.pod.zCG_m = optim.batteries.zCG_m(1);
...
optim.landingGear.zCG_m = [max(optim.boom.omlSectionsR_m{1}) max(optim.pod.omlSectionsR_m{1})];
