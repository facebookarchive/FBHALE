...
optim.wing.toc           = optim.wing.toc * (max(optim.wing.aero.airfoilADB.dim1.vector)-...
                            min(optim.wing.aero.airfoilADB.dim1.vector)) + min(optim.wing.aero.airfoilADB.dim1.vector);
...
% htail - elevon
optim.htail.controlSurface(1).name      = {'elevon'};
optim.htail.controlSurface(1).schedule  = [1 0 0];
optim.htail.controlSurface(1).yobo2     = [0 1];
optim.htail.controlSurface(1).csoc      = 1;
optim.htail.controlSurface(1).gearRatio = 1; 
optim.htail.controlSurface(1).maxDeflection_deg  = 9.1475; % CLmax + 2 degrees rad2deg(.7362/5.9015) + 2 = 9.1475

% Htail - elevator
optim.htail.controlSurface(2).name      = {'elevator'};
optim.htail.controlSurface(2).schedule  = [0 1 0];
optim.htail.controlSurface(2).yobo2     = [0 1];
optim.htail.controlSurface(2).csoc      = 1;
optim.htail.controlSurface(2).maxDeflection_deg  = 9.1475; % CLmax + 2 degrees rad2deg(.7362/5.9015) + 2 = 9.1475
