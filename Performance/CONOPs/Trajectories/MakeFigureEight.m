...
% This function creates the data relative to the figure eight trajectory. 
% The input a dictates the length of the half trajectory on the x-axis and 
% b does the same for the y axis.
% At each point we need the local:
%   1) s curvilinear abscissa
%   2) the ground path angle
%   3) the radius of curvature
function trajectory = MakeFigureEight(a, b)
...
trajectory.x_m 	= a * sin(2*theta_rad);
trajectory.y_m	= b * sin(theta_rad);
...
xp = 2 * a * cos(2*theta_rad);
yp = b * cos(theta_rad);
xpp = - 4 * trajectory.x_m;
ypp = - trajectory.y_m;
